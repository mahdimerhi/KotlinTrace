...
import sourceinfohook.KOTLINTRACE_IS_SIMULATOR
...
// Kotlin_getSourceInfo_Function. The simulator's default CoreSymbolication
// backend can never resolve source info (KT-75992), so on simulators we
// point the runtime at the bundled libbacktrace backend, which reads DWARF
// directly from the app binary or a dSYM next to the executable.
//
...
internal object SourceInfoHook {
    ...
    fun install() {
        // Device builds keep the default CoreSymbolication backend (it works
        // there and reads on-device symbolication, not DWARF).
        if (KOTLINTRACE_IS_SIMULATOR == 0) return
