...
Crashlytics adapter shipped: Kotlin/Native uncaught-exception hook, client-side frame
demangling, and real source `(File.kt:line)` on device and simulator — proven end-to-end
in the sample app (see [PROJECT.md](PROJECT.md)). Sentry/Bugsnag adapters are planned.
...
fast at startup with a clear error if Firebase Crashlytics is not linked.
...
extensions above (`installCrashlytics()`, and the future `installSentry()` /
